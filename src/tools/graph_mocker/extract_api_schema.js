...
module.exports = {
    ...
    getStringApiSchema: getStringApiSchema
}
 ...
 * @param {Api name} _apiName 
 */
function getStringApiSchema(_schemaProject, api_info){
    _schemaProject = (_schemaProject.endsWith(path.sep)) ? _schemaProject : _schemaProject + path.sep; //Add "/" if necessary to avoid furture appends
    var versionedSchema = {
        Schema:"",
        version: '1.0.0',
    ...
    if (!fs.existsSync(_schemaProject)) { 
        //console.log("ERROR: Could not find path " + _schemaProject);
        versionedSchema.adviseMessage = {
            code: 592,
            description: "Error can not find the path " + _schemaProject,
            level: 2
        }
        return versionedSchema;
    }
    var wApiPath = _schemaProject + api_info["product"] + path.sep + api_info["api"];
   
    wApiPath = (wApiPath.endsWith(path.sep)) ? wApiPath : wApiPath + path.sep; //Add "/" if necessary to avoid furture appends
    if (!fs.existsSync(wApiPath)) {
        versionedSchema.adviseMessage = {
            code: 593,
            description: "Can not find the api " + wApiPath,
            level: 2
        }
        return versionedSchema
    }
    ...
    versionedSchema.Schema = graphqllang.print(apiAST);
    return versionedSchema;
}